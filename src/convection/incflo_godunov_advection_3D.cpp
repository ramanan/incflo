                                       ...
                                       bool is_velocity )
{
    ...
    // Use PLM to generate Im and Ip */
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            Godunov_plm_fpu_ls_z(i, j, k, n, l_dt, dz, Imz(i,j,k,n), Ipz(i,j,k-1,n),
                                 q, wmac(i,j,k), pbc[n], flag, AMREX_D_DECL(fcx, fcy, fcz), ccc,
                                 AMREX_D_DECL(dlo.x, dlo.y, dlo.z), AMREX_D_DECL(dhi.x, dhi.y, dhi.z), is_velocity);
        });
    }
